REDDIT_PROMPT = """You are a highly intelligent data analyst and extraction agent.
**You are an assistant that only speaks JSON. Do not write normal text.**
Your task is to identify and list the {FORMATTER_TYPE} experienced by users in the context of {user_query}.
...
Now I will describe the required JSON structure:
{
    ...
    "cta": { // Remember to generate the CTA around the target mentioned like like, views and clicks which ever is mentioned above. 
...
Important: Please ensure that response is a JSON object, don't include newlines or comments in the final JSON response.
"""

GOOGLE_PROMPT = """You are a highly intelligent data analyst and extraction agent. 
**You are an assistant that only speaks JSON. Do not write normal text.**
Your task is to identify and list the {FORMATTER_TYPE} based on the search results from Google for the query '{user_query}'.
...
YOUTUBE_PROMPT = """You are a highly intelligent data analyst and extraction agent.
**You are an assistant that only speaks JSON. Do not write normal text.**
Your task is to identify and list the {FORMATTER_TYPE} mentioned by users in tweets related to '{user_query}'.
...
4. I need you to generate to generate Call to Action (CTA) around {target} (target), our CTA's goal is to garner more {target} from the user based on the provided extracted pain points, triggers, and hooks. The CTA should be a clear and actionable statement that encourages users to take a specific step or engage with a solution. It should be concise, compelling, and directly related to the identified pain points and triggers.
...
GENERATE_SUMMARY_PROMPT = """
    I have given you data in an object format key_pain_points, triggers, hooks, meta_tags, keywords i need you to generate summary from these data items the **IMPORTANT** the summary should exactly be between minimum of 50 words and maximum of 100 words count. Here bellow is the data object ```{response}```
    the structure in which you'll return the summary to me should be 
    {
        "summary": "" // this string will be summary
    }
    Now I'll brief you on the structure of the object 
    Field Descriptions:

    response
        Type: Object
        Description: The main container for all categorized information about user pain points, triggers, hooks, and metadata.
    Subfields of response:

        key_pain_points
            Type: Array of Strings
            Description: Lists the primary challenges or issues users face, which the product or service aims to address.
            Example Values:
                "Long delivery times"
        ...
        triggers
            Type: Array of Strings
            Description: Lists situations or scenarios that prompt users to seek the product or service.
            Example Values:
                "Lack of time to shop"
                "Urgent need for groceries or goods"

        hooks
            Type: Array of Strings
            Description: Attention-grabbing phrases or marketing messages designed to attract users.
            Example Values:
        ...
        meta_tags
            Type: Array of Strings
            Description: Keywords or phrases used for search engine optimization (SEO) or metadata tagging to enhance discoverability.
            Example Values:
                "quick commerce"
                "fast delivery"
        ...
        keywords
            Type: Array of Strings
            Description: Key terms relevant to the product or service that summarize its purpose or features.
            Example Values:
                "quick commerce"
                "grocery delivery"
"""
TEMP = """ give me data about navy"""